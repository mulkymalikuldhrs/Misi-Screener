...
class SignalAgent:
    ...
    def generate_signal(self, ticker: str = None) -> str:
        """
        Generates a trading signal (BUY, SELL, HOLD) for a specific ticker.
        """
        if ticker is None:
        ...
        logger.info(f"Latest RSI for {ticker} is {latest_rsi:.2f}")

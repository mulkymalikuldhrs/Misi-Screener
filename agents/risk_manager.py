...
class RiskManager:
    ...
    def calculate_stop_loss(self, ticker: str, entry_price: float, risk_params: Dict[str, Any]) -> float:
        ...
        if method == 'atr':
            ...
            print(f"Warning: Unknown stop-loss method '{method}'. Defaulting to fixed percent.")
            return self._calculate_fixed_percent_stop_loss(entry_price, risk_params)
    ...
    def _calculate_atr_stop_loss(self, ticker: str, entry_price: float, risk_params: Dict[str, Any]) -> float:
        ...
        if data.empty:
            print(f"RiskManager Warning: Could not fetch data for {ticker}. Using fixed percent stop-loss.")
            return self._calculate_fixed_percent_stop_loss(entry_price, risk_params)

        atr_series = self.technical_analyst.calculate_atr(data, period=atr_period)
